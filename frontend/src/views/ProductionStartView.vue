...
<template>
  <section class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        <p class="text-sm font-medium uppercase tracking-[0.22em] text-blue-600">Phase 4 migrated</p>
        <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Production Start</h1>
        <p class="mt-2 max-w-2xl text-slate-500">
          Start records are created from active Setting Die plans through existing Flask validation.
        </p>
      </div>
    ...
    <div v-else-if="!starts.length" class="shell-card p-10 text-center">
      ...
      <h2 class="text-lg font-semibold text-slate-900">No Production Start records found</h2>
      <p class="mt-2 text-sm text-slate-500">Add a Production Start from an available active Setting Die plan.</p>
    </div>
